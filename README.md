...
1. **Group-level `CLAUDE.md`** at `<group>/CLAUDE.md`. Tells Claude in plain
   ...
   cwd to find this. Starts with a one-line HTML-comment header so the
   regeneration policy is visible to anyone reading the source:

   ```markdown
   <!-- Generated by `worktrees feature-xyz` on 2026-05-14. Edit freely — re-runs leave this file alone. To refresh from template: `rm CLAUDE.md && worktrees feature-xyz`. -->

   # Worktree group: feature-xyz
   …
   ```
